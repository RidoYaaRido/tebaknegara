...
function isCorrect(answer) {
  ...
  if (isCorrect) {
    ...
    if (streak === 3) {
      ...
      correctAnswer.textContent = "+5 Bonus Point!!";
...
function scoreCheck() {
  ...
  let savedHighScore = localStorage.getItem("disimpanSkor Tinggi");
  ...
  if (savedHighScore === "null" || score > savedHighScore || score === 100) {
    // then save new score
    localStorage.setItem("disimpanSkor Tinggi", score);
    highScore.textContent = score;
    // change text to High Score or Perfect Score if 100 points
    highScoreText.textContent =
      score === 100 ? "Perfect Score!!!" : "Skor Tinggi Baru!!";
...
function setSocialLinks() {
  let shareText = `Saya baru saja mencetak score ${score} poin dalam Tantangan Kuis Negara! Bisakah kamu berbuat lebih baik?`;
