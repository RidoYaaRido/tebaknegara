...
regionSelectors.forEach((btn) =>
  btn.addEventListener("click", (e) => {
    ...
    setTimeout(() => {
      ...
      console.log(`Pilih Negara Setelah Wilayah: ${countryList.length}`);
    }, 200);
  })
);
  ...
  btn.addEventListener("click", (e) => {
    ...
    setTimeout(() => {
      ...
      for (let country of selectedCountries) {
        console.log(`nama: ${country.name}, ibukota: ${country.capital}, population: ${country.population.toLocaleString()}`);
      }
...
function nameQuestion(answers, name) {
  ...
  answerInput.focus();
  question.innerText = "Apa nama negara ini?";
...
function capitalQuestion(answers, name) {
  ...
  let capital = answers[1]
    ...
    .map((word) => word.replace(word[0], word[0].toUpperCase()))
    .join(" ");
  question.innerText = `Apa ibukotanya ${name}?`;
...
function populationQuestion(answers, name) {
  ...
  question.innerText = `Berapa jumlah penduduknya ${name}?`;
  ...
  console.log(`\nPilihan populasi: ${popOptions}`);
  console.log(`Populasi sebenarnya: ${answers[2]}`);
  console.log(`Populasi Sebenarnya ada dalam opsi? ${popOptions.includes(answers[2]) ? "Yes" : "No"}`);
  console.log(`Indeks populasi sebenarnya${popOptions.indexOf(answers[2])}`);
...
function submitAnswer(answer, displayAnswer) {
  ...
  correctAnswer.innerHTML = `Jawaban yang benar adalah <span class="answer__answer--country">${displayAnswer}</span>`;
}
...
function isCorrect(answer) {
  ...
  if (isCorrect) {
    ...
    answerResult.textContent = "Benar!!!";
    ...
    answerResult.textContent = "salah";
